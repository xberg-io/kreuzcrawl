...
pub fn crawlenginehandle_crawl_stream_start(
    ...
    let core_req: kreuzcrawl::CrawlStreamRequest = req
        .map(|s| serde_json::from_str::<kreuzcrawl::CrawlStreamRequest>(&s))
        .transpose()
...
pub fn crawlenginehandle_batch_crawl_stream_start(
    ...
    let core_req: kreuzcrawl::BatchCrawlStreamRequest = req
        .map(|s| serde_json::from_str::<kreuzcrawl::BatchCrawlStreamRequest>(&s))
        .transpose()
        .map_err(|e| e.to_string())?
        .unwrap_or_default();
    let runtime = std::sync::Arc::new(
...
pub fn interact_async(
    ...
    let actions_core: Vec<kreuzcrawl::PageAction> = actions
        .map(|s| serde_json::from_str::<Vec<kreuzcrawl::PageAction>>(&s).map_err(|e| e.to_string()))
        .transpose()?
        .unwrap_or_default();
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        std::thread::Builder::new()
...
/// Convert a `kreuzcrawl::SsrfError` error to a Rustler error string.
#[allow(dead_code)]
fn ssrf_error_to_rustler_err(e: kreuzcrawl::SsrfError) -> String {
    e.to_string()
}
