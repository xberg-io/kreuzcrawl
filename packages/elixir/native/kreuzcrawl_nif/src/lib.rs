...
// alef:hash:271925d1212728d1e7517577d6706bfbaa5929a5bfe78bad4375ec2e2e5581fa
...
impl BrowserConfig {
    pub fn new(opts: std::collections::HashMap<String, rustler::Term>) -> Self {
        Self {
            ...
            stealth: opts.get("stealth").and_then(|t| t.decode().ok()).unwrap_or(false),
...
impl CrawlConfig {
    pub fn new(opts: std::collections::HashMap<String, rustler::Term>) -> Self {
        Self {
            ...
            proxy: opts.get("proxy").and_then(|t| t.decode().ok()),
            bypass: opts.get("bypass").and_then(|t| t.decode().ok()),
            user_agents: opts
                .get("user_agents")
                .and_then(|t| t.decode().ok())
                .unwrap_or_default(),
...
pub fn interact_async(
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let result = rt
                .block_on(async {
                    kreuzcrawl::interact(&engine.inner, &url, actions.into_iter().map(Into::into).collect()).await
                })
                .map_err(|e| e.to_string())?;
            Ok(result.into())
        })
