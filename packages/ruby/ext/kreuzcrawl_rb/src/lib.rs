...
// alef:hash:085215b16e4e32bc6f3056222e8b9245cedb31815ef130d9bbb982e40951a33e
...
fn interact(engine: CrawlEngineHandle, url: String, actions: Vec<PageAction>) -> Result<InteractionResult, Error> {
    ...
    let result = rt
        .block_on(async { kreuzcrawl::interact(&engine.inner, &url, actions).await })
        .map_err(|e| {
            magnus::Error::new(
                unsafe { Ruby::get_unchecked() }.exception_runtime_error(),
                e.to_string(),
...
impl From<kreuzcrawl::CrawlEvent> for CrawlEvent {
    fn from(val: kreuzcrawl::CrawlEvent) -> Self {
        match val {
            kreuzcrawl::CrawlEvent::Page { result } => Self::Page { result: result.into() },
