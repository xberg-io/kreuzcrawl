...
object Crawlberg {
    ...
    fun createEngine(config: CrawlConfig? = null): CrawlEngineHandle =
        CrawlEngineHandle(
            CrawlbergBridge.nativeCreateEngine(config?.let { mapper.writeValueAsString(it) } ?: "")
        )
    ...
    fun interact(
        ...
        val resultJson =
            CrawlbergBridge.nativeInteract(engine.handle, url, mapper.writeValueAsString(actions))
    ...
    fun batchScrape(engine: CrawlEngineHandle, urls: List<String>): BatchScrapeResults {
        val resultJson =
            CrawlbergBridge.nativeBatchScrape(engine.handle, mapper.writeValueAsString(urls))
    ...
    fun batchCrawl(engine: CrawlEngineHandle, urls: List<String>): BatchCrawlResults {
        val resultJson =
            CrawlbergBridge.nativeBatchCrawl(engine.handle, mapper.writeValueAsString(urls))
