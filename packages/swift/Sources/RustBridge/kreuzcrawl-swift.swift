...
extension BrowserConfig {
    public convenience init<GenericIntoRustString: IntoRustString>(_ mode: BrowserMode, _ endpoint: Optional<GenericIntoRustString>, _ timeout: UInt64, _ wait: BrowserWait, _ wait_selector: Optional<GenericIntoRustString>, _ extra_wait: Optional<UInt64>) {
        self.init(ptr: __swift_bridge__$BrowserConfig$new({mode.isOwned = false; return mode.ptr;}(), { if let rustString = optionalStringIntoRustString(endpoint) { rustString.isOwned = false; return rustString.ptr } else { return nil } }(), timeout, {wait.isOwned = false; return wait.ptr;}(), { if let rustString = optionalStringIntoRustString(wait_selector) { rustString.isOwned = false; return rustString.ptr } else { return nil } }(), extra_wait.intoFfiRepr()))
    }
}
...
extension CrawlPageResult {
    public convenience init<GenericIntoRustString: IntoRustString>(_ url: GenericIntoRustString, _ normalized_url: GenericIntoRustString, _ status_code: UInt16, _ content_type: GenericIntoRustString, _ html: GenericIntoRustString, _ body_size: UInt, _ metadata: PageMetadata, _ links: RustVec<LinkInfo>, _ images: RustVec<ImageInfo>, _ feeds: RustVec<FeedInfo>, _ json_ld: RustVec<JsonLdEntry>, _ depth: UInt, _ stayed_on_domain: Bool, _ was_skipped: Bool, _ is_pdf: Bool, _ detected_charset: Optional<GenericIntoRustString>, _ markdown: Optional<MarkdownResult>, _ extracted_data: Optional<GenericIntoRustString>, _ extraction_meta: Optional<ExtractionMeta>, _ downloaded_document: Optional<DownloadedDocument>) {
...
extension CrawlResult {
    public convenience init<GenericIntoRustString: IntoRustString>(_ pages: RustVec<CrawlPageResult>, _ final_url: GenericIntoRustString, _ redirect_count: UInt, _ was_skipped: Bool, _ error: Optional<GenericIntoRustString>, _ cookies: RustVec<CookieInfo>, _ normalized_urls: RustVec<GenericIntoRustString>) {
        self.init(ptr: __swift_bridge__$CrawlResult$new({ let val = pages; val.isOwned = false; return val.ptr }(), { let rustString = final_url.intoRustString(); rustString.isOwned = false; return rustString.ptr }(), redirect_count, was_skipped, { if let rustString = optionalStringIntoRustString(error) { rustString.isOwned = false; return rustString.ptr } else { return nil } }(), { let val = cookies; val.isOwned = false; return val.ptr }(), { let val = normalized_urls; val.isOwned = false; return val.ptr }()))
    }
}
...
extension MarkdownResultRef {
    ...
    public func citations() -> Optional<CitationResult> {
