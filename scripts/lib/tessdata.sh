...
setup_tessdata() {
  ...
  macOS | Darwin)
    if [ -d "/opt/homebrew/opt/tesseract/share/tessdata" ]; then
      ...
      export TESSDATA_PREFIX="$HOME/Library/Application Support/kreuzcrawl-tesseract/tessdata"
    fi
    ;;
  Windows | MINGW* | MSYS* | CYGWIN*)
    export TESSDATA_PREFIX="${APPDATA:-${USERPROFILE:-}}/kreuzcrawl-tesseract/tessdata"
    ;;
  *)
