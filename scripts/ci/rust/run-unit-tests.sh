...
# NOTE: We intentionally avoid `--all-features` for the `kreuzcrawl` crate because
TEST_LOG="/tmp/cargo-test-$$.log"

if ! {
  echo "=== cargo test -p kreuzcrawl --features full ==="
  RUST_BACKTRACE=full cargo test -p kreuzcrawl --features full --verbose

  echo "=== cargo test --workspace (all features, excluding kreuzcrawl) ==="
  extra_excludes=()
  ...
  RUST_BACKTRACE=full cargo test \
    --workspace \
    --exclude kreuzcrawl \
    --exclude kreuzcrawl-e2e-generator \
