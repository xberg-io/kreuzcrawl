...
pushd "$tmp" >/dev/null
cp "$tarball" ./kreuzcrawl-node.tgz
cp -R "${GITHUB_WORKSPACE}/crates/kreuzcrawl-node" ./kreuzcrawl-node-pkg

node -e "
  const fs = require('fs');
  const pkg = JSON.parse(fs.readFileSync('kreuzcrawl-node-pkg/package.json', 'utf8'));
...
pushd kreuzcrawl-node-pkg >/dev/null
