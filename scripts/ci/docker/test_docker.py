...
RESULTS_FILE = Path("/tmp/kreuzcrawl-docker-test-results.json")
...
class TestRunner:
    ...
    def container_name(self) -> str:
        name = f"kreuzcrawl-test-{int(time.time())}-{random.randint(0, 99999)}"
...
def test_version(t: TestRunner) -> None:
    ...
    if "kreuzcrawl" in out.lower():
        t.pass_test()
    else:
        t.fail_test("CLI version", f"Expected 'kreuzcrawl' in output, got: {out}")
...
def test_security_nonroot(t: TestRunner) -> None:
    ...
    if user == "kreuzcrawl":
        ...
        t.fail_test("Non-root user", f"Container running as: {user} (expected: kreuzcrawl)")
