...
$stage = "kreuzcrawl-cli-$target"
Remove-Item -Recurse -Force $stage -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Path $stage | Out-Null

$candidates = @(
  ("target/" + $target + "/release/kreuzcrawl.exe"),
  ("target/release/kreuzcrawl.exe"),
  ("target/" + $target + "/release/kreuzcrawl-cli.exe"),
  ("target/release/kreuzcrawl-cli.exe")
)
