...
if [[ "$OS" == "windows" ]]; then
  EXT_FILE="kreuzcrawl_php.${EXT_SUFFIX}"
else
  EXT_FILE="libkreuzcrawl_php.${EXT_SUFFIX}"
fi
...
if [[ -d "${TARGET_DIR}" ]]; then
  ...
  shopt -s nullglob
  files=("${TARGET_DIR}"/*kreuzcrawl* "${TARGET_DIR}"/*.dll "${TARGET_DIR}"/*.so "${TARGET_DIR}"/*.dylib)
  if [[ ${#files[@]} -gt 0 ]]; then
    ls -la "${files[@]}"
  else
    echo "No kreuzcrawl binaries found"
  fi
else
...
if [[ ! -f "${TARGET_DIR}/${EXT_FILE}" ]]; then
  ...
  for ext in .dll .so .dylib; do
    for prefix in libkreuzcrawl_php kreuzcrawl_php; do
...
PKG_NAME="kreuzcrawl-${VERSION}-${PLATFORM}"
...
This is a pre-built PIE package for the Kreuzcrawl PHP extension.
