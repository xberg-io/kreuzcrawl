...
for rid in linux-arm64 linux-x64 osx-arm64 win-x64; do
  ...
  if unzip -l "$pkg" | grep -E "runtimes/${rid}/native/.*kreuzcrawl_ffi\\.(dll|so|dylib)"; then
    echo "  ✓ Found kreuzcrawl_ffi for $rid"
  else
    echo "  ✗ Missing kreuzcrawl_ffi binary for $rid" >&2
