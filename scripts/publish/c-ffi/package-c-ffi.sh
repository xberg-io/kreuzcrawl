...
# Package kreuzcrawl-ffi C distribution tarball.
#
# Usage:
#   package-c-ffi.sh <platform-label> <version> <output-dir>
#
# The script builds kreuzcrawl-ffi in release mode, then creates a tarball with
# the structure expected by downstream C/C++ consumers:
#
#   kreuzcrawl-ffi-<version>-<platform>/
#     include/kreuzcrawl.h
#     lib/libkreuzcrawl_ffi.{so|dylib|dll}
#     lib/libkreuzcrawl_ffi.{a|lib}
#     lib/pkgconfig/kreuzcrawl.pc
#     cmake/kreuzcrawl-ffi-config.cmake
#     cmake/kreuzcrawl-ffi-config-version.cmake
#     LICENSE
...
ffi_crate_dir="${repo_root}/crates/kreuzcrawl-ffi"
...
linux-*)
  shared_lib="libkreuzcrawl_ffi.so"
  static_lib="libkreuzcrawl_ffi.a"
  libs_private="-lpthread -ldl -lm"
  ;;
macos-*)
  shared_lib="libkreuzcrawl_ffi.dylib"
  static_lib="libkreuzcrawl_ffi.a"
  libs_private="-framework CoreFoundation -framework Security -lpthread"
  ;;
windows-*)
  shared_lib="kreuzcrawl_ffi.dll"
  static_lib="kreuzcrawl_ffi.lib"
...
echo "Building kreuzcrawl-ffi (release) ..."
cargo build -p kreuzcrawl-ffi --release "${FEATURES_ARGS[@]}"
...
find_lib() {
  ...
  local alt
  case "$name" in
  kreuzcrawl_ffi.dll) alt="libkreuzcrawl_ffi.dll" ;;
  kreuzcrawl_ffi.lib) alt="libkreuzcrawl_ffi.a" ;;
  *) alt="" ;;
  esac
...
header_path="${ffi_crate_dir}/kreuzcrawl.h"
...
# ---------------------------------------------------------------------------
dist_name="kreuzcrawl-ffi-${version}-${platform_label}"
...
# Header
cp "$header_path" "${stage_dir}/include/kreuzcrawl.h"
...
if [[ "$platform_label" == windows-* ]]; then
  for dir in "$target_release" ${target_release_fallback:+"$target_release_fallback"}; do
    if [ -f "${dir}/kreuzcrawl_ffi.dll.lib" ]; then
      cp "${dir}/kreuzcrawl_ffi.dll.lib" "${stage_dir}/lib/"
      break
    elif [ -f "${dir}/libkreuzcrawl_ffi.dll.a" ]; then
      cp "${dir}/libkreuzcrawl_ffi.dll.a" "${stage_dir}/lib/kreuzcrawl_ffi.dll.lib"
      break
    fi
  done
fi
...
# ---------------------------------------------------------------------------
cat >"${stage_dir}/lib/pkgconfig/kreuzcrawl.pc" <<EOF
prefix=/usr/local
exec_prefix=\${prefix}
...
# CMake config files
# ---------------------------------------------------------------------------
cp "${ffi_crate_dir}/cmake/kreuzcrawl-ffi-config.cmake" "${stage_dir}/cmake/kreuzcrawl-ffi-config.cmake"

# Generate version cmake with the correct version from script argument
cat >"${stage_dir}/cmake/kreuzcrawl-ffi-config-version.cmake" <<EOF
# kreuzcrawl-ffi version compatibility check
#
