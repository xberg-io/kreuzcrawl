 ...
 * alef:hash:6249e373c833c080761218961c24fdbf40d2d2521efa2b885f14aafe2541c596
...
void test_interact_execute_js(void) {
    ...
    const char* actions_json = "[{\"script\":\"return document.title\",\"type\":\"executeJs\"}]";
    KCRAWLInteractionResult* result = kcrawl_interact(engine, url, actions_json);
    assert(result != NULL && "expected call to succeed");
    char* action_results_json = kcrawl_interaction_result_action_results(result);
    assert(action_results_json != NULL);
...
void test_interact_invalid_selector(void) {
    ...
    assert(str_trim_eq(interaction_action_results_0_action_type, "click") == 0 && "equals assertion failed");
    assert(interaction_action_results_0_error != NULL && strstr(interaction_action_results_0_error, "could not find") != NULL && "expected to contain substring");
    free(interaction_action_results_0_success);
    free(interaction_action_results_0_action_type);
