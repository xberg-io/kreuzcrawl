...
// alef:hash:17fa5c5d18216b8545bbcc6a30d04a4f6de32e0f4adbba31da37344ce2692d46
...
final class MetadataTest extends TestCase
{
    ...
    public function test_metadata_keywords_author(): void
    {
        ...
        $result = Kreuzcrawl::scrape($engine, $url);
        $this->assertEquals(200, $result->statusCode);
        $this->assertEquals("Comprehensive Metadata Test Page", trim($result->metadata->title));
        ...
        $this->assertEquals("Jane Developer", trim($result->metadata->author));
        $this->assertNotEmpty($result->metadata->viewport);
        $this->assertEquals("kreuzcrawl/1.0", trim($result->metadata->generator));
        $this->assertEquals("#ff6600", trim($result->metadata->themeColor));
        $this->assertEquals("index, follow", trim($result->metadata->robots));
        $this->assertEquals("en", trim($result->metadata->htmlLang));
        $this->assertEquals("ltr", trim($result->metadata->htmlDir));
    ...
    public function test_metadata_og_video_audio(): void
    {
        ...
        $result = Kreuzcrawl::scrape($engine, $url);
        $this->assertEquals(200, $result->statusCode);
        $this->assertEquals("https://example.com/video.mp4", trim($result->metadata->ogVideo));
        $this->assertEquals("https://example.com/audio.mp3", trim($result->metadata->ogAudio));
