...
// alef:hash:ececa7acd0a6e6ec950596ae6a86002978b3324580173e188735411b9cef5a97
...
final class MetadataTest extends TestCase
{
    ...
    public function test_metadata_keywords_author(): void
    {
        ...
        $result = Kreuzcrawl::scrape($engine, $url);
        $this->assertEquals(200, $result->statusCode);
        $this->assertEquals("Comprehensive Metadata Test Page", trim($result->getMetadata()->title));
        $this->assertNotEmpty($result->getMetadata()->canonicalUrl);
        $this->assertNotEmpty($result->getMetadata()->keywords);
        $this->assertStringContainsString("rust", $result->getMetadata()->keywords);
        $this->assertEquals("Jane Developer", trim($result->getMetadata()->author));
        $this->assertNotEmpty($result->getMetadata()->viewport);
        $this->assertEquals("kreuzcrawl/1.0", trim($result->getMetadata()->generator));
        $this->assertEquals("#ff6600", trim($result->getMetadata()->themeColor));
        $this->assertEquals("index, follow", trim($result->getMetadata()->robots));
    ...
    public function test_metadata_word_count(): void
    {
        ...
        $result = Kreuzcrawl::scrape($engine, $url);
        $this->assertEquals(200, $result->statusCode);
        // skipped: field 'computed.word_count' not available on result type        // skipped: field 'computed.word_count' not available on result type
    }
