// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:201177ab1fb2fdec3556e24178fc176a8245e0c8a6a32a882f4cc3ded3e5ac8b
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code

package dev.kreuzberg.kreuzcrawl.e2e;

import static org.junit.jupiter.api.Assertions.*;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.fasterxml.jackson.datatype.jdk8.Jdk8Module;
import dev.kreuzberg.kreuzcrawl.CrawlConfig;
import dev.kreuzberg.kreuzcrawl.Kreuzcrawl;
import org.junit.jupiter.api.BeforeAll;
import org.junit.jupiter.api.Test;

/** E2e tests for category: browser. */
public class BrowserTest {
  private static final ObjectMapper MAPPER = new ObjectMapper()
      .registerModule(new Jdk8Module())
      .setPropertyNamingStrategy(
          com.fasterxml.jackson.databind.PropertyNamingStrategies.SNAKE_CASE);

  @BeforeAll
  static void initEnv() {
    if (System.getProperty("KREUZCRAWL_ALLOW_PRIVATE_NETWORK") == null) {
      System.setProperty("KREUZCRAWL_ALLOW_PRIVATE_NETWORK", "true");
    }
  }

  @Test
  void testBrowserConfigAutoNoFeature() throws Exception {
    // Browser mode 'never' prevents browser use even when JS render hint is set
    var engineConfig = MAPPER.readValue("{\"browser\":{\"mode\":\"never\"}}", CrawlConfig.class);
    var engine = Kreuzcrawl.createEngine(engineConfig);
    String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
        + "/fixtures/browser_config_auto_no_feature";
    var result = Kreuzcrawl.scrape(engine, url);
    assertEquals(200, result.statusCode());
    assertEquals(true, result.jsRenderHint());
    assertEquals(false, result.browserUsed());
  }

  @Test
  void testBrowserConfigNeverMode() throws Exception {
    // Browser mode 'never' prevents browser fallback even for SPA shell content
    var engineConfig = MAPPER.readValue("{\"browser\":{\"mode\":\"never\"}}", CrawlConfig.class);
    var engine = Kreuzcrawl.createEngine(engineConfig);
    String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
        + "/fixtures/browser_config_never_mode";
    var result = Kreuzcrawl.scrape(engine, url);
    assertEquals(200, result.statusCode());
    assertEquals(true, result.jsRenderHint());
    assertEquals(false, result.browserUsed());
  }

  @Test
  void testBrowserCrawlModeAlways() throws Exception {
    // Crawl with browser mode 'always' follows links using browser rendering
    var engineConfig = MAPPER.readValue(
        "{\"browser\":{\"mode\":\"always\"},\"max_depth\":1,\"respect_robots_txt\":false}",
        CrawlConfig.class);
    var engine = Kreuzcrawl.createEngine(engineConfig);
    String url = System.getProperty(
        "mockServer.browser_crawl_mode_always",
        System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
            + "/fixtures/browser_crawl_mode_always");
    var result = Kreuzcrawl.crawl(engine, url);
    assertTrue(
        result.pages().size() >= 2,
        "expected >= 2"); // skipped: field 'browser_used' not available on result type
  }

  @Test
  void testBrowserCrawlWafFallback() throws Exception {
    // Crawl with browser mode 'auto' falls back to browser when encountering WAF 403
    var engineConfig = MAPPER.readValue(
        "{\"browser\":{\"mode\":\"auto\"},\"max_depth\":1,\"respect_robots_txt\":false}",
        CrawlConfig.class);
    var engine = Kreuzcrawl.createEngine(engineConfig);
    String url = System.getProperty(
        "mockServer.browser_crawl_waf_fallback",
        System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
            + "/fixtures/browser_crawl_waf_fallback");
    var result = Kreuzcrawl.crawl(engine, url);
    assertTrue(result.pages().size() >= 1, "expected >= 1");
  }

  @Test
  void testBrowserDetectMinimalPage() throws Exception {
    // Does NOT flag a short but real content page as needing JS rendering
    var engine = Kreuzcrawl.createEngine(null);
    String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
        + "/fixtures/browser_detect_minimal_page";
    var result = Kreuzcrawl.scrape(engine, url);
    assertEquals(200, result.statusCode());
    assertEquals(false, result.jsRenderHint());
    assertEquals(false, result.browserUsed());
  }

  @Test
  void testBrowserDetectNextEmpty() throws Exception {
    // Detects Next.js page with __NEXT_DATA__ but no rendered content as needing JS rendering
    var engineConfig = MAPPER.readValue("{\"browser\":{\"mode\":\"never\"}}", CrawlConfig.class);
    var engine = Kreuzcrawl.createEngine(engineConfig);
    String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
        + "/fixtures/browser_detect_next_empty";
    var result = Kreuzcrawl.scrape(engine, url);
    assertEquals(200, result.statusCode());
    assertEquals(true, result.jsRenderHint());
    assertEquals(false, result.browserUsed());
  }

  @Test
  void testBrowserDetectNextRendered() throws Exception {
    // Does NOT flag Next.js page with full SSR content as needing JS rendering
    var engine = Kreuzcrawl.createEngine(null);
    String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
        + "/fixtures/browser_detect_next_rendered";
    var result = Kreuzcrawl.scrape(engine, url);
    assertEquals(200, result.statusCode());
    assertFalse(result.html().isEmpty(), "expected non-empty value");
    assertEquals(false, result.jsRenderHint());
    assertEquals(false, result.browserUsed());
  }

  @Test
  void testBrowserDetectNormalPage() throws Exception {
    // Does NOT flag a normal server-rendered page as needing JS rendering
    var engine = Kreuzcrawl.createEngine(null);
    String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
        + "/fixtures/browser_detect_normal_page";
    var result = Kreuzcrawl.scrape(engine, url);
    assertEquals(200, result.statusCode());
    assertEquals(false, result.jsRenderHint());
    assertEquals(false, result.browserUsed());
  }

  @Test
  void testBrowserDetectNuxtShell() throws Exception {
    // Detects Nuxt SPA shell with empty #__nuxt div as needing JS rendering
    var engineConfig = MAPPER.readValue("{\"browser\":{\"mode\":\"never\"}}", CrawlConfig.class);
    var engine = Kreuzcrawl.createEngine(engineConfig);
    String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
        + "/fixtures/browser_detect_nuxt_shell";
    var result = Kreuzcrawl.scrape(engine, url);
    assertEquals(200, result.statusCode());
    assertEquals(true, result.jsRenderHint());
    assertEquals(false, result.browserUsed());
  }

  @Test
  void testBrowserDetectReactShell() throws Exception {
    // Detects React SPA shell with empty #root div as needing JS rendering
    var engineConfig = MAPPER.readValue("{\"browser\":{\"mode\":\"never\"}}", CrawlConfig.class);
    var engine = Kreuzcrawl.createEngine(engineConfig);
    String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
        + "/fixtures/browser_detect_react_shell";
    var result = Kreuzcrawl.scrape(engine, url);
    assertEquals(200, result.statusCode());
    assertFalse(result.html().isEmpty(), "expected non-empty value");
    assertEquals(true, result.jsRenderHint());
    assertEquals(false, result.browserUsed());
  }

  @Test
  void testBrowserDetectVueShell() throws Exception {
    // Detects Vue SPA shell with empty #app div as needing JS rendering
    var engineConfig = MAPPER.readValue("{\"browser\":{\"mode\":\"never\"}}", CrawlConfig.class);
    var engine = Kreuzcrawl.createEngine(engineConfig);
    String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
        + "/fixtures/browser_detect_vue_shell";
    var result = Kreuzcrawl.scrape(engine, url);
    assertEquals(200, result.statusCode());
    assertEquals(true, result.jsRenderHint());
    assertEquals(false, result.browserUsed());
  }

  @Test
  void testBrowserExtraWait() throws Exception {
    // Browser extra_wait adds additional time after network_idle to ensure all async operations
    // complete
    var engineConfig = MAPPER.readValue(
        "{\"browser\":{\"extra_wait\":200,\"mode\":\"always\",\"wait\":\"network_idle\"}}",
        CrawlConfig.class);
    var engine = Kreuzcrawl.createEngine(engineConfig);
    String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
        + "/fixtures/browser_extra_wait";
    var result = Kreuzcrawl.scrape(engine, url);
    assertEquals(true, result.browserUsed());
  }

  @Test
  void testBrowserFallbackSpaRender() throws Exception {
    // Browser auto re-fetches SPA shell when JS rendering is detected
    var engineConfig = MAPPER.readValue("{\"browser\":{\"mode\":\"always\"}}", CrawlConfig.class);
    var engine = Kreuzcrawl.createEngine(engineConfig);
    String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
        + "/fixtures/browser_fallback_spa_render";
    var result = Kreuzcrawl.scrape(engine, url);
    assertEquals(true, result.jsRenderHint());
    assertEquals(true, result.browserUsed());
  }

  @Test
  void testBrowserFallbackWafBlocked() throws Exception {
    // Browser fallback is used when browser mode is always, simulating WAF-blocked scenario
    var engineConfig = MAPPER.readValue("{\"browser\":{\"mode\":\"always\"}}", CrawlConfig.class);
    var engine = Kreuzcrawl.createEngine(engineConfig);
    String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
        + "/fixtures/browser_fallback_waf_blocked";
    var result = Kreuzcrawl.scrape(engine, url);
    assertEquals(true, result.browserUsed());
  }

  @Test
  void testBrowserModeAlways() throws Exception {
    // Browser mode 'always' uses browser even for normal server-rendered pages
    var engineConfig = MAPPER.readValue("{\"browser\":{\"mode\":\"always\"}}", CrawlConfig.class);
    var engine = Kreuzcrawl.createEngine(engineConfig);
    String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
        + "/fixtures/browser_mode_always";
    var result = Kreuzcrawl.scrape(engine, url);
    assertEquals(true, result.browserUsed());
  }

  @Test
  void testBrowserProfileBasic() throws Exception {
    // Browser profile configuration persists and reuses browser state across crawl sessions
    var engineConfig = MAPPER.readValue(
        "{\"browser\":{\"mode\":\"always\"},\"browser_profile\":\"test-profile\"}",
        CrawlConfig.class);
    var engine = Kreuzcrawl.createEngine(engineConfig);
    String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
        + "/fixtures/browser_profile_basic";
    var result = Kreuzcrawl.scrape(engine, url);
    assertEquals(true, result.browserUsed());
    assertEquals(200, result.statusCode());
  }

  @Test
  void testBrowserWaitFixed() throws Exception {
    // Browser wait strategy 'fixed' waits for a specific duration after page navigation
    var engineConfig = MAPPER.readValue(
        "{\"browser\":{\"extra_wait\":100,\"mode\":\"always\",\"wait\":\"fixed\"}}",
        CrawlConfig.class);
    var engine = Kreuzcrawl.createEngine(engineConfig);
    String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
        + "/fixtures/browser_wait_fixed";
    var result = Kreuzcrawl.scrape(engine, url);
    assertEquals(true, result.browserUsed());
    assertEquals(200, result.statusCode());
  }

  @Test
  void testBrowserWaitSelector() throws Exception {
    // Browser wait strategy 'selector' waits for specific CSS selector before considering page
    // loaded
    var engineConfig = MAPPER.readValue(
        "{\"browser\":{\"mode\":\"always\",\"wait\":\"selector\",\"wait_selector\":\"#content\"}}",
        CrawlConfig.class);
    var engine = Kreuzcrawl.createEngine(engineConfig);
    String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL"))
        + "/fixtures/browser_wait_selector";
    var result = Kreuzcrawl.scrape(engine, url);
    assertEquals(true, result.browserUsed());
    assertEquals(200, result.statusCode());
  }
}
