...
// alef:hash:1dae6446a47d988a2102479f4ae4a37a53217a9f51073487db745597dd20815c
...
assertEquals(200, result.statusCode());assertFalse(result.markdown().content().isEmpty(), "expected non-empty value");assertTrue(result.markdown().content().contains("("), "expected to contain: " + "(");
    ...
    void testCitationsDuplicateUrls() throws Exception {
        ...
        var result = Kreuzcrawl.scrape(engine, url);
assertEquals(200, result.statusCode());assertFalse(result.markdown().content().isEmpty(), "expected non-empty value");assertTrue(java.util.Optional.ofNullable(result.markdown().citations()).orElse(""), "expected true");
    }
...
assertEquals(200, result.statusCode());assertEquals("Test", java.util.Optional.ofNullable(result.metadata().title()).orElse("").trim());assertFalse(result.html().isEmpty(), "expected non-empty value");assertFalse(result.markdown().content().isEmpty(), "expected non-empty value");assertTrue(result.markdown().content().contains("Hello World"), "expected to contain: " + "Hello World");
    ...
    void testMarkdownCrawlAllPages() throws Exception {
        ...
        String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL")) + "/fixtures/markdown_crawl_all_pages";
        var result = Kreuzcrawl.scrape(engine, url);
    ...
    void testMarkdownFitContent() throws Exception {
        ...
        var engine = Kreuzcrawl.createEngine(null);
        String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL")) + "/fixtures/markdown_fit_content";
        var result = Kreuzcrawl.scrape(engine, url);
