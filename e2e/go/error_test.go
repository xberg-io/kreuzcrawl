...
// alef:hash:0c5c8d1b5c0a60ab5797401e489a54041db9997dad8e100a05bcfdfb1b6b5cfe
...
func Test_ErrorBrowserLaunchFailure(t *testing.T) {
	...
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"browser":{"mode":"always"},"browser_launch_timeout_ms":1}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
...
func Test_ErrorBrowserPageTimeout(t *testing.T) {
	...
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"browser":{"mode":"always"},"page_load_timeout_ms":1}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
