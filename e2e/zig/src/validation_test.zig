...
// alef:hash:3e639d686808dd80407c84ebe07393ff239a2c70350344961119825ad06ac09d
...
test "validation_empty_batch_urls" {
    // batch operation with empty batch_urls array is rejected
    var gpa: std.heap.DebugAllocator(.{}) = .init;
    defer _ = gpa.deinit();
    const allocator = gpa.allocator();

    const url = if (std.c.getenv("MOCK_SERVER_VALIDATION_EMPTY_BATCH_URLS")) |_pf| try std.fmt.allocPrint(allocator, "{s}", .{std.mem.span(_pf)}) else try std.fmt.allocPrint(allocator, "{s}/fixtures/validation_empty_batch_urls", .{if (std.c.getenv("MOCK_SERVER_URL")) |v| std.mem.span(v) else "http://localhost:8080"});
    defer allocator.free(url);
