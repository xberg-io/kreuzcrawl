...
// alef:hash:396e3b7d578aa48433540b42a94be8596037c87e8d7477cbafb5beb55e4a7f6e
...
test "interact_execute_js" {
    ...
    defer allocator.free(url);
    const _result_json = try kreuzcrawl.interact("{\"browser\":{\"mode\":\"always\"}}", url, "[{\"script\":\"return document.title\",\"type\":\"executeJs\"}]");
    defer std.heap.c_allocator.free(_result_json);
    var _parsed = try std.json.parseFromSlice(std.json.Value, allocator, _result_json, .{});
    defer _parsed.deinit();
    const result = &_parsed.value;
...
test "interact_invalid_selector" {
    ...
    try testing.expectEqualStrings("click", std.mem.trim(u8, result.object.get("interaction").?.object.get("action_results").?.array.items[0].object.get("action_type").?.string, " \n\r\t"));
    {
        ...
        const _js = if (_jv == .string) _jv.string else try std.json.Stringify.valueAlloc(std.heap.c_allocator, _jv, .{});
        defer if (_jv != .string) std.heap.c_allocator.free(_js);
        try testing.expect(std.mem.indexOf(u8, _js, "not found") != null);
    }
}
...
test "interact_screenshot" {
    ...
    defer allocator.free(url);
    const _result_json = try kreuzcrawl.interact("{\"browser\":{\"mode\":\"always\"}}", url, "[{\"full_page\":false,\"selector\":\"#target\",\"type\":\"screenshot\"}]");
    defer std.heap.c_allocator.free(_result_json);
    var _parsed = try std.json.parseFromSlice(std.json.Value, allocator, _result_json, .{});
    defer _parsed.deinit();
    const result = &_parsed.value;
    ...
    try testing.expect(result.object.get("interaction").?.object.get("action_results").?.array.items[0].object.get("data").?.bool);
}
