...
# alef:hash:e6c5abce63878ac27743a5ae776b89d1d9d2baf3f521c4509e3a4b27dc220ea4
...
test_browser_config_auto_no_feature() {
  ...
  local val_browser_js_render_hint
  val_browser_js_render_hint=$(echo "$output" | jq -r '.browser.js_render_hint')
  assert_equals "$val_browser_js_render_hint" 'true' 'browser.js_render_hint'
  local val_browser_browser_used
  val_browser_browser_used=$(echo "$output" | jq -r '.browser.browser_used')
  assert_equals "$val_browser_browser_used" 'false' 'browser.browser_used'
}
...
test_browser_crawl_mode_always() {
  ...
  local output
  output=$(kreuzcrawl scrape "${MOCK_SERVER_BROWSER_CRAWL_MODE_ALWAYS:-${MOCK_SERVER_URL}/fixtures/browser_crawl_mode_always}" --config '{"browser":{"mode":"always"},"max_depth":1,"respect_robots_txt":false}' --format json --browser-mode never)
...
test_browser_crawl_waf_fallback() {
  ...
  local output
  output=$(kreuzcrawl scrape "${MOCK_SERVER_BROWSER_CRAWL_WAF_FALLBACK:-${MOCK_SERVER_URL}/fixtures/browser_crawl_waf_fallback}" --config '{"browser":{"mode":"auto"},"max_depth":1,"respect_robots_txt":false}' --format json --browser-mode never)
...
test_browser_detect_minimal_page() {
  ...
  local val_browser_js_render_hint
  val_browser_js_render_hint=$(echo "$output" | jq -r '.browser.js_render_hint')
  assert_equals "$val_browser_js_render_hint" 'false' 'browser.js_render_hint'
  local val_browser_browser_used
  val_browser_browser_used=$(echo "$output" | jq -r '.browser.browser_used')
  assert_equals "$val_browser_browser_used" 'false' 'browser.browser_used'
}
...
test_browser_extra_wait() {
  ...
  local val_browser_browser_used
  val_browser_browser_used=$(echo "$output" | jq -r '.browser.browser_used')
  assert_equals "$val_browser_browser_used" 'true' 'browser.browser_used'
}

test_browser_fallback_spa_render() {
  # Browser auto re-fetches SPA shell when JS rendering is detected
  local output
  output=$(kreuzcrawl scrape "${MOCK_SERVER_BROWSER_FALLBACK_SPA_RENDER:-${MOCK_SERVER_URL}/fixtures/browser_fallback_spa_render}" --config '{"browser":{"mode":"auto"}}' --format json --browser-mode never)
...
test_browser_fallback_waf_blocked() {
  ...
  local output
  output=$(kreuzcrawl scrape "${MOCK_SERVER_BROWSER_FALLBACK_WAF_BLOCKED:-${MOCK_SERVER_URL}/fixtures/browser_fallback_waf_blocked}" --config '{"browser":{"mode":"auto"}}' --format json --browser-mode never)
