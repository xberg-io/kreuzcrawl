...
# alef:hash:6e605c7f251004e8d5f46547f9b3a85c52d8a79637024f30ebe0a9109eddbaab
...
test_filter_bm25_empty_query() {
  ...
  local output
  output=$(kreuzcrawl scrape "${MOCK_SERVER_FILTER_BM25_EMPTY_QUERY:-${MOCK_SERVER_URL}/fixtures/filter_bm25_empty_query}" --config '{"max_depth":1}' --format json --browser-mode never)
...
test_filter_bm25_threshold_zero() {
  ...
  local output
  output=$(kreuzcrawl scrape "${MOCK_SERVER_FILTER_BM25_THRESHOLD_ZERO:-${MOCK_SERVER_URL}/fixtures/filter_bm25_threshold_zero}" --config '{"max_depth":1}' --format json --browser-mode never)
...
test_filter_noop_passes_all() {
  ...
  local output
  output=$(kreuzcrawl scrape "${MOCK_SERVER_FILTER_NOOP_PASSES_ALL:-${MOCK_SERVER_URL}/fixtures/filter_noop_passes_all}" --config '{"max_depth":1}' --format json --browser-mode never)
