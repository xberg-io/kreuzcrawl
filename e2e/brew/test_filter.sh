...
test_filter_bm25_empty_query() {
  ...
  kreuzcrawl crawl "${MOCK_SERVER_FILTER_BM25_EMPTY_QUERY:-${MOCK_SERVER_URL}/fixtures/filter_bm25_empty_query}" --config '{"max_depth":1}' --format json --browser-mode never >/dev/null
...
test_filter_bm25_threshold_zero() {
  ...
  kreuzcrawl crawl "${MOCK_SERVER_FILTER_BM25_THRESHOLD_ZERO:-${MOCK_SERVER_URL}/fixtures/filter_bm25_threshold_zero}" --config '{"max_depth":1}' --format json --browser-mode never >/dev/null
...
test_filter_noop_passes_all() {
  ...
  kreuzcrawl crawl "${MOCK_SERVER_FILTER_NOOP_PASSES_ALL:-${MOCK_SERVER_URL}/fixtures/filter_noop_passes_all}" --config '{"max_depth":1}' --format json --browser-mode never >/dev/null
