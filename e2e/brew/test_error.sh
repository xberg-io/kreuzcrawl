...
# alef:hash:0cb8f84c55c34fff3a6930974019a9de2b899615a7bb709894fb0376a80f365c
...
test_error_browser_launch_failure() {
  ...
  if kreuzcrawl scrape "${MOCK_SERVER_ERROR_BROWSER_LAUNCH_FAILURE:-${MOCK_SERVER_URL}/fixtures/error_browser_launch_failure}" --config '{"browser":{"mode":"always"},"browser_launch_timeout_ms":1}' --format json --browser-mode never >/dev/null 2>&1; then
    echo 'FAIL [error]: expected command to fail but it succeeded' >&2
    return 1
  fi
}
...
test_error_browser_page_timeout() {
  ...
  if kreuzcrawl scrape "${MOCK_SERVER_ERROR_BROWSER_PAGE_TIMEOUT:-${MOCK_SERVER_URL}/fixtures/error_browser_page_timeout}" --config '{"browser":{"mode":"always"},"page_load_timeout_ms":1}' --format json --browser-mode never >/dev/null 2>&1; then
    echo 'FAIL [error]: expected command to fail but it succeeded' >&2
    return 1
  fi
}
...
test_error_empty_response() {
  ...
  local val_error_is_error
  val_error_is_error=$(echo "$output" | jq -r '.error.is_error')
  assert_equals "$val_error_is_error" 'false' 'error.is_error'
}
...
test_error_waf_blocked() {
  ...
  if kreuzcrawl scrape "${MOCK_SERVER_ERROR_WAF_BLOCKED:-${MOCK_SERVER_URL}/fixtures/error_waf_blocked}" --format json --browser-mode never >/dev/null 2>&1; then
    echo 'FAIL [error]: expected command to fail but it succeeded' >&2
    return 1
  fi
}
