...
// alef:hash:2b34b9a14579a09d24ef94a144a2ee790ae39868c45cab77616723d9bff24e35
...
describe("batch", () => {
  it("batch_crawl_basic: Batch crawl of 2 seed URLs with links to discover", async () => {
    ...
    await batchCrawl(engine, ["/seed1", "/seed2"]); // skipped: field 'batch.completed_count' not available on result type
  ...
  it("batch_crawl_partial_failure: Batch crawl where one seed URL returns 404 error", async () => {
    const engineConfig = { maxDepth: 1, respectRobotsTxt: false };
    const engine = createEngine(engineConfig);
    await batchCrawl(engine, ["/good_seed", "/bad_seed"]); // skipped: field 'batch.completed_count' not available on result type
    // skipped: field 'batch.failed_count' not available on result type
    // skipped: field 'batch.total_count' not available on result type
  });
