...
// alef:hash:967de2ed8f402d97302706a3b361ede3aa6e1ddd00039c18abb4a6b8e0dcf986
...
describe("batch", () => {
  it("batch_crawl_basic: Batch crawl of 2 seed URLs with links to discover", async () => {
    ...
    const result = await batchCrawl(engine, ["/seed1", "/seed2"]);
  ...
  it("batch_crawl_partial_failure: Batch crawl where one seed URL returns 404 error", async () => {
    const engineConfig = { maxDepth: 1, respectRobotsTxt: false };
    const engine = createEngine(engineConfig);
    const result = await batchCrawl(engine, ["/good_seed", "/bad_seed"]);
  ...
  it("batch_scrape_with_config: Batch scrape with aggressive preprocessing configuration", async () => {
    ...
    const engine = createEngine(engineConfig);
    const result = await batchScrape(engine, ["/article1", "/article2"]);
    expect(result.completedCount).toBe(2);
    expect(result.failedCount).toBe(0);
    expect(result.totalCount).toBe(2);
  });
  it("scrape_batch_basic: Batch scrape of multiple URLs all succeeding", async () => {
    const engine = createEngine(null);
    const result = await batchScrape(engine, ["/page1", "/page2", "/page3"]);
  ...
  it("scrape_batch_partial_failure: Batch scrape with one URL failing returns partial results", async () => {
    const engine = createEngine(null);
    const result = await batchScrape(engine, ["/good1", "/bad", "/good2"]);
    expect(result.completedCount).toBe(2);
    expect(result.failedCount).toBe(1);
    expect(result.totalCount).toBe(3);
  });
  it("scrape_batch_progress: Batch scrape results include specific URL", async () => {
    const engine = createEngine(null);
    const result = await batchScrape(engine, ["/target", "/other"]);
    expect(result.totalCount).toBe(2);
    expect(result.results).toContain("/target");
  });
});
