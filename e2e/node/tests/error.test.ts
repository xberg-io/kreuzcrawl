...
// alef:hash:6f889392e89075cb88ee6d910f1095a78235bfbd0d96726f93e301df52dac175
...
describe("error", () => {
  ...
  it("error_browser_launch_failure: Browser launch fails when browser mode is always but browser is unavailable", async () => {
    await expect(async () => {
      const engineConfig = { browser: { mode: "always" }, browserLaunchTimeoutMs: 1 };
      const engine = createEngine(engineConfig);
  ...
  it("error_browser_page_timeout: Browser page load times out", async () => {
    await expect(async () => {
      const engineConfig = { browser: { mode: "always" }, pageLoadTimeoutMs: 1 };
      const engine = createEngine(engineConfig);
