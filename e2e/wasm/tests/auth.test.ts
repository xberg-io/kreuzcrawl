...
describe('auth', () => {
  it('auth_basic_http: Sends HTTP Basic authentication header', async () => {
    const engine = createEngine({ auth: { password: "testpass", type: "basic", username: "testuser" }, respect_robots_txt: false });
  ...
  it('auth_bearer_token: Sends Bearer token in Authorization header', async () => {
    const engine = createEngine({ auth: { token: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.test", type: "bearer" }, respect_robots_txt: false });
  ...
  it('auth_custom_header: Sends authentication via custom header (X-API-Key)', async () => {
    const engine = createEngine({ auth: { name: "X-API-Key", type: "header", value: "sk-test-key-12345" }, respect_robots_txt: false });
