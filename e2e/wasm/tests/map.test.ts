...
// alef:hash:70ad2d900698f751558b7c89448409769e2f0fbc4a3ab6827b5c53a369f5c418
...
describe("map", () => {
  ...
  it("map_include_subdomains: Includes subdomain URLs in URL map discovery", async () => {
    ...
    expect(
      result.urls.some((item) =>
        _alefE2eItemTexts(item).some((text) => text.includes("blog.example.com")),
      ),
    ).toBe(true);
  });
  ...
  it("map_search_filter: Filters map results by search keyword", async () => {
    ...
    expect(Number(result.urls.length)).toBeGreaterThanOrEqual(2);
    expect(
      result.urls.some((item) => _alefE2eItemTexts(item).some((text) => text.includes("blog"))),
    ).toBe(true);
  });
});
