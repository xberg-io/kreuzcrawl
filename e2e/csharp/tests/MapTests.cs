...
// alef:hash:7c4ccbb4f685f99a1c280c9b169493d5ac9cf864e36778f46c5d7f212d3b0e97
...
namespace Kreuzcrawl
{
    ...
    Assert.Contains("blog.example.com", JsonSerializer.Serialize(result.Urls).ToLower());
    ...
    Assert.True(result.Urls.Count <= 5, "expected <= 5");
        ...
        public async Task Test_MapSearchFilter()
        {
            ...
            var result = await KreuzcrawlLib.MapUrlsAsync(engine, url);
    Assert.True(result.Urls.Count >= 2, "expected >= 2");
    Assert.Contains("blog", JsonSerializer.Serialize(result.Urls).ToLower());
