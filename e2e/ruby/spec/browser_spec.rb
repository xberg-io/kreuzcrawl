...
# alef:hash:a82a78eb2347f1ddb6ca299d9d0d5f4fb34a718ca5a76c4280c28bf7da539051
...
RSpec.describe 'browser' do
  it 'browser_config_auto_no_feature: Browser mode \'auto\' without browser feature enabled does not use browser' do
    ...
    expect(result.browser.js_render_hint.to_s).to be(true)
    expect(result.browser.browser_used.to_s).to be(false)
  end
  ...
  it 'browser_detect_minimal_page: Does NOT flag a short but real content page as needing JS rendering' do
    ...
    result = Kreuzcrawl.scrape_async(engine, url)
    expect(result.status_code).to eq(200)
    expect(result.browser.js_render_hint.to_s).to be(false)
    expect(result.browser.browser_used.to_s).to be(false)
  end
  ...
  it 'browser_extra_wait: Browser extra_wait adds additional time after network_idle to ensure all async operations complete' do
    ...
    result = Kreuzcrawl.scrape_async(engine, url)
    expect(result.browser.browser_used.to_s).to be(true)
  end
