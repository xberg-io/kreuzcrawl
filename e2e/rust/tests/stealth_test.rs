...
async fn test_stealth_ua_rotation_config() {
    ...
    let url = format!("{}/fixtures/{}", std::env::var("MOCK_SERVER_URL").expect("MOCK_SERVER_URL not set"), "stealth_ua_rotation_config");
...
async fn test_stealth_ua_rotation_round_robin() {
    ...
    let engine = create_engine(Some(engine_config)).expect("handle creation should succeed");
    let url = format!("{}/fixtures/{}", std::env::var("MOCK_SERVER_URL").expect("MOCK_SERVER_URL not set"), "stealth_ua_rotation_round_robin");
...
async fn test_stealth_ua_rotation_single_domain() {
    ...
    let engine = create_engine(Some(engine_config)).expect("handle creation should succeed");
    let url = format!("{}/fixtures/{}", std::env::var("MOCK_SERVER_URL").expect("MOCK_SERVER_URL not set"), "stealth_ua_rotation_single_domain");
    let result = scrape(&engine, &url).await.expect("should succeed");
    assert_eq!(result.status_code, 200, "equals assertion failed");
    // skipped: field 'pages.length' not available on result type
}
