...
async fn test_rate_limit_adaptive_backoff() {
    ...
    let url = format!("{}/fixtures/{}", std::env::var("MOCK_SERVER_URL").expect("MOCK_SERVER_URL not set"), "rate_limit_adaptive_backoff");
...
async fn test_rate_limit_per_domain() {
    ...
    let engine = create_engine(Some(engine_config)).expect("handle creation should succeed");
    let url = format!("{}/fixtures/{}", std::env::var("MOCK_SERVER_URL").expect("MOCK_SERVER_URL not set"), "rate_limit_per_domain");
    let result = scrape(&engine, &url).await.expect("should succeed");
...
async fn test_rate_limit_robots_crawl_delay() {
    ...
    let engine = create_engine(Some(engine_config)).expect("handle creation should succeed");
    let url = format!("{}/fixtures/{}", std::env::var("MOCK_SERVER_URL").expect("MOCK_SERVER_URL not set"), "rate_limit_robots_crawl_delay");
    let result = scrape(&engine, &url).await.expect("should succeed");
    // skipped: field 'pages.length' not available on result type
    assert_eq!(result.status_code, 200, "equals assertion failed");
}
