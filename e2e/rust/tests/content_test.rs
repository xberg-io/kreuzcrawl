...
async fn test_content_204_no_content() {
    ...
    let url = format!("{}/fixtures/{}", std::env::var("MOCK_SERVER_URL").expect("MOCK_SERVER_URL not set"), "content_204_no_content");
...
async fn test_content_main_only() {
    ...
    let engine_config: CrawlConfig = serde_json::from_str("{\"main_content_only\":true,\"respect_robots_txt\":false}").expect("config should parse");
    let engine = create_engine(Some(engine_config)).expect("handle creation should succeed");
