...
// alef:hash:ffbad33cf3ab1b8f17721d9832ab7b5e1a889bf5905052b850a4a50019f530f2
...
void main() {
  ...
  test('Concurrent crawling fetches all pages with max_concurrent workers', () async {
    ...
    final url = _mockServerFor("concurrent_basic", "MOCK_SERVER_CONCURRENT_BASIC");
  ...
  test('Concurrent depth=2 crawl correctly fans out and deduplicates across levels', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("concurrent_depth_two_fan_out", "MOCK_SERVER_CONCURRENT_DEPTH_TWO_FAN_OUT");
    final result = await KreuzcrawlBridge.crawl(engine, url);
  ...
  test('Concurrent crawling does not exceed max_pages limit even with high concurrency', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("concurrent_max_pages_exact", "MOCK_SERVER_CONCURRENT_MAX_PAGES_EXACT");
    final result = await KreuzcrawlBridge.crawl(engine, url);
  ...
  test('Concurrent crawl handles partial failures gracefully', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"max_concurrent":3,"max_depth":1}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("concurrent_partial_errors", "MOCK_SERVER_CONCURRENT_PARTIAL_ERRORS");
    final result = await KreuzcrawlBridge.crawl(engine, url);
  ...
  test('Concurrent crawling respects max_pages limit', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("concurrent_respects_max_pages", "MOCK_SERVER_CONCURRENT_RESPECTS_MAX_PAGES");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, lessThanOrEqualTo(3));
  });
