// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:0ba927fedb193da697a0c28a4daac1722416be2cd8934e54222921ac9084b0c3
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code
// Issues & docs: https://github.com/sample_crate-dev/alef
// ignore_for_file: unused_local_variable

import 'package:test/test.dart';
import 'dart:io';
import 'package:kreuzcrawl/kreuzcrawl.dart';
import 'package:kreuzcrawl/src/kreuzcrawl_bridge_generated/frb_generated.dart' show RustLib;
import 'dart:async';
import 'dart:convert';

// E2e tests for category: robots

String _alefE2eText(Object? value) {
  if (value == null) return '';
  // Check if it's an enum by examining its toString representation.
  final str = value.toString();
  if (str.contains('.')) {
    // Enum.toString() returns 'EnumName.variantName'. Extract the variant name.
    final parts = str.split('.');
    if (parts.length == 2) {
      final variantName = parts[1];
      // Convert camelCase variant names to snake_case for serde compatibility.
      // E.g. 'toolCalls' -> 'tool_calls', 'stop' -> 'stop'.
      return _camelToSnake(variantName);
    }
  }
  return str;
}

String _camelToSnake(String camel) {
  final buffer = StringBuffer();
  for (int i = 0; i < camel.length; i++) {
    final char = camel[i];
    if (char.contains(RegExp(r'[A-Z]'))) {
      if (i > 0) buffer.write('_');
      buffer.write(char.toLowerCase());
    } else {
      buffer.write(char);
    }
  }
  return buffer.toString();
}

Process? _mockServer;
String? _spawnedMockServerUrl;
final Map<String, String> _spawnedMockServers = {};

String _mockServerUrl() => _spawnedMockServerUrl ?? Platform.environment['MOCK_SERVER_URL'] ?? 'http://localhost:8080';

String _mockServerFor(String fixtureId, String envKey) => _spawnedMockServers[fixtureId] ?? Platform.environment[envKey] ?? '${_mockServerUrl()}/fixtures/$fixtureId';

void main() {
  setUpAll(() async {
    await RustLib.init();
    if (Platform.environment['MOCK_SERVER_URL'] == null) {
      final _bin = Platform.script.resolve('../rust/target/release/mock-server').toFilePath();
      final _fixturesDir = Platform.script.resolve('../../fixtures').toFilePath();
      if (File(_bin).existsSync()) {
        _mockServer = await Process.start(_bin, [_fixturesDir], mode: ProcessStartMode.normal);
        final _ready = Completer<void>();
        _mockServer!.stdout
            .transform(utf8.decoder)
            .transform(const LineSplitter())
            .listen((_line) {
          final _trimmed = _line.trim();
          if (_trimmed.startsWith('MOCK_SERVER_URL=')) {
            _spawnedMockServerUrl = _trimmed.substring('MOCK_SERVER_URL='.length);
          } else if (_trimmed.startsWith('MOCK_SERVERS=')) {
            final _json = _trimmed.substring('MOCK_SERVERS='.length);
            try {
              final _parsed = jsonDecode(_json) as Map<String, dynamic>;
              _parsed.forEach((k, v) {
                _spawnedMockServers[k] = v as String;
              });
            } catch (_) {}
            if (!_ready.isCompleted) _ready.complete();
          } else if (_spawnedMockServerUrl != null) {
            if (!_ready.isCompleted) _ready.complete();
          }
        }, onDone: () {
          if (!_ready.isCompleted) _ready.complete();
        });
        await _ready.future.timeout(const Duration(seconds: 30), onTimeout: () {});
      }
    }
  });

  tearDownAll(() async {
    final server = _mockServer;
    if (server != null) {
      server.kill();
      await server.exitCode;
    }
  });

  test('Permissive robots.txt allows all paths', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"respect_robots_txt":true}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("robots_allow_all", "MOCK_SERVER_ROBOTS_ALLOW_ALL");
    final result = await KreuzcrawlBridge.scrape(engine, url);
    expect(result.isAllowed, equals(true));
  });

  test('Allow directive overrides Disallow for specific paths', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"respect_robots_txt":true}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("robots_allow_override", "MOCK_SERVER_ROBOTS_ALLOW_OVERRIDE");
    final result = await KreuzcrawlBridge.scrape(engine, url);
    expect(result.isAllowed, equals(true));
  });

  test('Correctly parses robots.txt with inline and line comments', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"respect_robots_txt":true,"user_agent":"kreuzcrawl"}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("robots_comments_handling", "MOCK_SERVER_ROBOTS_COMMENTS_HANDLING");
    final result = await KreuzcrawlBridge.scrape(engine, url);
    expect(result.isAllowed, equals(true));
  });

  test('Respects crawl-delay directive from robots.txt', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"respect_robots_txt":true,"user_agent":"kreuzcrawl"}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("robots_crawl_delay", "MOCK_SERVER_ROBOTS_CRAWL_DELAY");
    final result = await KreuzcrawlBridge.scrape(engine, url);
    expect(result.crawlDelay, equals(2));
  });

  test('Robots.txt disallows specific paths', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"respect_robots_txt":true}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("robots_disallow_path", "MOCK_SERVER_ROBOTS_DISALLOW_PATH");
    final result = await KreuzcrawlBridge.scrape(engine, url);
    expect(result.isAllowed, equals(false));
  });

  test('Detects nofollow meta robots tag and skips link extraction', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"respect_robots_txt":true}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("robots_meta_nofollow", "MOCK_SERVER_ROBOTS_META_NOFOLLOW");
    final result = await KreuzcrawlBridge.scrape(engine, url);
    expect(result.nofollowDetected, equals(true));
  });

  test('Detects noindex meta robots tag in HTML page', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"respect_robots_txt":true}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("robots_meta_noindex", "MOCK_SERVER_ROBOTS_META_NOINDEX");
    final result = await KreuzcrawlBridge.scrape(engine, url);
    expect(result.noindexDetected, equals(true));
  });

  test('Missing robots.txt (404) allows all crawling', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"respect_robots_txt":true}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("robots_missing_404", "MOCK_SERVER_ROBOTS_MISSING_404");
    final result = await KreuzcrawlBridge.scrape(engine, url);
    expect(result.isAllowed, equals(true));
  });

  test('Picks the most specific user-agent block from robots.txt', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"respect_robots_txt":true,"user_agent":"SpecificBot"}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("robots_multiple_user_agents", "MOCK_SERVER_ROBOTS_MULTIPLE_USER_AGENTS");
    final result = await KreuzcrawlBridge.scrape(engine, url);
    expect(result.isAllowed, equals(true));
  });

  test('Parses request-rate directive from robots.txt', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"respect_robots_txt":true,"user_agent":"kreuzcrawl"}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("robots_request_rate", "MOCK_SERVER_ROBOTS_REQUEST_RATE");
    final result = await KreuzcrawlBridge.scrape(engine, url);
    expect(result.crawlDelay, equals(5));
    expect(result.isAllowed, equals(true));
  });

  test('Discovers sitemap URL from Sitemap directive in robots.txt', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"respect_robots_txt":true}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("robots_sitemap_directive", "MOCK_SERVER_ROBOTS_SITEMAP_DIRECTIVE");
    final result = await KreuzcrawlBridge.scrape(engine, url);
    expect(result.isAllowed, equals(true));
  });

  test('Matches user-agent specific rules in robots.txt', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"respect_robots_txt":true,"user_agent":"KreuzcrawlBot"}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("robots_user_agent_specific", "MOCK_SERVER_ROBOTS_USER_AGENT_SPECIFIC");
    final result = await KreuzcrawlBridge.scrape(engine, url);
    expect(result.isAllowed, equals(false));
  });

  test('Handles wildcard Disallow patterns in robots.txt', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"respect_robots_txt":true}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("robots_wildcard_paths", "MOCK_SERVER_ROBOTS_WILDCARD_PATHS");
    final result = await KreuzcrawlBridge.scrape(engine, url);
    expect(result.isAllowed, equals(false));
  });

  test('Respects X-Robots-Tag HTTP header directives', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"respect_robots_txt":true}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("robots_x_robots_tag", "MOCK_SERVER_ROBOTS_X_ROBOTS_TAG");
    final result = await KreuzcrawlBridge.scrape(engine, url);
    expect(result.xRobotsTag.toString().trim(), equals('noindex, nofollow'.toString().trim()));
    expect(result.noindexDetected, equals(true));
    expect(result.nofollowDetected, equals(true));
  });

}
