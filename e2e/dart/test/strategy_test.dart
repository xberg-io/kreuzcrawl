...
// alef:hash:134531e6a4c738a82e450048f90fc2d9c4848d95911b4c916fda43242371374f
...
void main() {
  ...
  test('Adaptive strategy stops early when encountering saturation (duplicate content)', () async {
    ...
    final url = _mockServerFor("strategy_adaptive_saturation", "MOCK_SERVER_STRATEGY_ADAPTIVE_SATURATION");
  ...
  test('Adaptive strategy crawls more pages when content is diverse', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("strategy_adaptive_window", "MOCK_SERVER_STRATEGY_ADAPTIVE_WINDOW");
    final result = await KreuzcrawlBridge.crawl(engine, url);
  ...
  test('BestFirst strategy always processes the seed URL first', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("strategy_best_first_seed", "MOCK_SERVER_STRATEGY_BEST_FIRST_SEED");
    final result = await KreuzcrawlBridge.crawl(engine, url);
  ...
  test('BFS strategy visits pages in breadth-first order', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("strategy_bfs_default_order", "MOCK_SERVER_STRATEGY_BFS_DEFAULT_ORDER");
    final result = await KreuzcrawlBridge.crawl(engine, url);
  ...
  test('DFS strategy visits pages in depth-first order', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"max_concurrent":1,"max_depth":2}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("strategy_dfs_depth_first", "MOCK_SERVER_STRATEGY_DFS_DEPTH_FIRST");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, equals(5));
    // skipped: field 'strategy.crawl_order' not available on dart result type
  });
