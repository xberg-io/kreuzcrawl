...
// alef:hash:468d88a37f2fd945881ecdbbbfcf551e5d3cfd43fb7f7e5d2489f24012623f82
...
void main() {
  ...
  test('Concurrent crawl respects max_depth limit', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_concurrent_depth", "MOCK_SERVER_CRAWL_CONCURRENT_DEPTH");
  ...
  test('Respects max concurrent requests limit during crawl', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_concurrent_limit", "MOCK_SERVER_CRAWL_CONCURRENT_LIMIT");
    final result = await KreuzcrawlBridge.crawl(engine, url);
  ...
  test('Concurrent crawl respects max_pages budget', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_concurrent_max_pages", "MOCK_SERVER_CRAWL_CONCURRENT_MAX_PAGES");
    final result = await KreuzcrawlBridge.crawl(engine, url);
  ...
  test('Sends custom headers on all crawl requests', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_custom_headers", "MOCK_SERVER_CRAWL_CUSTOM_HEADERS");
    final result = await KreuzcrawlBridge.crawl(engine, url);
  ...
  test('Follows links one level deep from start page', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"max_depth":1,"respect_robots_txt":false}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_depth_one", "MOCK_SERVER_CRAWL_DEPTH_ONE");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, equals(3));
    expect(result.stayedOnDomain, equals(true));
  });
  ...
  test('Crawls in breadth-first order, processing depth-0 pages before depth-1', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_depth_priority", "MOCK_SERVER_CRAWL_DEPTH_PRIORITY");
    final result = await KreuzcrawlBridge.crawl(engine, url);
  ...
  test('Crawls 3 levels deep (depth 0, 1, 2)', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"max_depth":2,"respect_robots_txt":false}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_depth_two", "MOCK_SERVER_CRAWL_DEPTH_TWO");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, equals(3));
  ...
  test('Depth=2 crawl follows a chain of links across three levels', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_depth_two_chain", "MOCK_SERVER_CRAWL_DEPTH_TWO_CHAIN");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, equals(3));
  });
  ...
  test('Normalizes double slashes in URL paths (//page to /page)', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"max_depth":1,"respect_robots_txt":false}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_double_slash_normalization", "MOCK_SERVER_CRAWL_DOUBLE_SLASH_NORMALIZATION");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, equals(2));
  });
  ...
  test('Crawl completes when child page has no outgoing links', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"max_concurrent":1,"max_depth":2}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_empty_page_no_links", "MOCK_SERVER_CRAWL_EMPTY_PAGE_NO_LINKS");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, equals(2));
  });
  ...
  test('Skips URLs matching the exclude path pattern', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_exclude_path_pattern", "MOCK_SERVER_CRAWL_EXCLUDE_PATH_PATTERN");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, equals(2));
  });
  ...
  test('External links are discovered but not followed when stay_on_domain is true', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_external_links_ignored", "MOCK_SERVER_CRAWL_EXTERNAL_LINKS_IGNORED");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, equals(2));
    expect(result.stayedOnDomain, equals(true));
  });
  ...
  test('Strips #fragment from URLs for deduplication', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"max_depth":1,"respect_robots_txt":false}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_fragment_stripping", "MOCK_SERVER_CRAWL_FRAGMENT_STRIPPING");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, equals(2));
  });
  ...
  test('Only follows URLs matching the include path pattern', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_include_path_pattern", "MOCK_SERVER_CRAWL_INCLUDE_PATH_PATTERN");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, equals(2));
  });
  ...
  test('max_depth=0 crawls only the seed page with no link following', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_max_depth_zero", "MOCK_SERVER_CRAWL_MAX_DEPTH_ZERO");
    final result = await KreuzcrawlBridge.crawl(engine, url);
  ...
  test('Stops crawling at page budget limit', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_max_pages", "MOCK_SERVER_CRAWL_MAX_PAGES");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, lessThanOrEqualTo(3));
  });
  ...
  test('Crawl handles links to non-HTML content types gracefully', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_mixed_content_types", "MOCK_SERVER_CRAWL_MIXED_CONTENT_TYPES");
    final result = await KreuzcrawlBridge.crawl(engine, url);
  ...
  test('Multiple linked pages with redirects are handled during crawl traversal', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"max_concurrent":1,"max_depth":1}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_multiple_redirects_in_traversal", "MOCK_SERVER_CRAWL_MULTIPLE_REDIRECTS_IN_TRAVERSAL");
    final result = await KreuzcrawlBridge.crawl(engine, url);
  ...
  test('Deduplicates URLs with same query params in different order', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"max_depth":1,"respect_robots_txt":false}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_query_param_dedup", "MOCK_SERVER_CRAWL_QUERY_PARAM_DEDUP");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, equals(2));
  });
  ...
  test('Links that redirect are followed during crawl traversal', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"max_concurrent":1,"max_depth":1}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_redirect_in_traversal", "MOCK_SERVER_CRAWL_REDIRECT_IN_TRAVERSAL");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, greaterThanOrEqualTo(1));
  });
  ...
  test('Page linking to itself does not cause infinite crawl loop', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"max_concurrent":1,"max_depth":1}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_self_link_no_loop", "MOCK_SERVER_CRAWL_SELF_LINK_NO_LOOP");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, equals(2));
  });
  ...
  test('Does not follow external links when stay_on_domain is true', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_stay_on_domain", "MOCK_SERVER_CRAWL_STAY_ON_DOMAIN");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, equals(2));
    expect(result.stayedOnDomain, equals(true));
  });
  ...
  test('Stays on exact domain and skips subdomain links', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_subdomain_exclusion", "MOCK_SERVER_CRAWL_SUBDOMAIN_EXCLUSION");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, equals(2));
    expect(result.stayedOnDomain, equals(true));
  });
  ...
  test('Crawls subdomains when allow_subdomains is enabled', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_subdomain_inclusion", "MOCK_SERVER_CRAWL_SUBDOMAIN_INCLUSION");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, greaterThanOrEqualTo(2));
  });
  ...
  test('Deduplicates /page and /page/ as the same URL', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"max_depth":1,"respect_robots_txt":false}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_trailing_slash_dedup", "MOCK_SERVER_CRAWL_TRAILING_SLASH_DEDUP");
    final result = await KreuzcrawlBridge.crawl(engine, url);
    expect(result.pages.length, equals(2));
  });
  ...
  test('Deduplicates URLs that differ only by fragment or query params', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"max_depth":1,"respect_robots_txt":false}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("crawl_url_deduplication", "MOCK_SERVER_CRAWL_URL_DEDUPLICATION");
    final result = await KreuzcrawlBridge.crawl(engine, url);
