...
void main() {
  ...
  test('Rate limiter adds delay between requests to the same domain', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("rate_limit_basic_delay", "MOCK_SERVER_RATE_LIMIT_BASIC_DELAY");
  ...
  test('Per-domain rate limiting applies delay between requests to same domain', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("rate_limit_per_domain", "MOCK_SERVER_RATE_LIMIT_PER_DOMAIN");
    final result = await KreuzcrawlBridge.crawl(engine, url);
  ...
  test('Respects Crawl-delay directive in robots.txt', () async {
    ...
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("rate_limit_robots_crawl_delay", "MOCK_SERVER_RATE_LIMIT_ROBOTS_CRAWL_DELAY");
    final result = await KreuzcrawlBridge.crawl(engine, url);
  ...
  test('Rate limiter with zero delay does not slow crawling', () async {
    final engineConfig = await createCrawlConfigFromJson(json: r'{"max_depth":1}');
    final engine = await KreuzcrawlBridge.createEngine(engineConfig);
    final url = _mockServerFor("rate_limit_zero_no_delay", "MOCK_SERVER_RATE_LIMIT_ZERO_NO_DELAY");
    final result = await KreuzcrawlBridge.crawl(engine, url);
