...
// alef:hash:beee2e43b971b6961f12e3952734d495367aeef56caa797688cc00246a3801cf
...
impl WasmBrowserConfig {
    ...
    pub fn new(
        ...
        WasmBrowserConfig {
            ...
            stealth: stealth.unwrap_or(false),
            proxy,
...
impl WasmCrawlConfig {
    ...
    #[wasm_bindgen(setter)]
    pub fn set_bypass(&mut self, value: Option<String>) {
        self.bypass = value;
    }
...
impl From<CrawlConfigInput> for kreuzcrawl::CrawlConfig {
    fn from(val: CrawlConfigInput) -> Self {
        ...
        if let Some(v) = val.include_paths {
            out.include_paths = v.into();
        }
        if let Some(v) = val.exclude_paths {
            out.exclude_paths = v.into();
        }
        ...
        if let Some(v) = val.bypass {
            out.bypass = serde_json::from_str(&v).unwrap_or_default();
        }
        if let Some(v) = val.user_agents {
            out.user_agents = v.into();
        }
        ...
        if let Some(v) = val.document_mime_types {
            out.document_mime_types = v.into();
        }
...
impl From<kreuzcrawl::CrawlConfig> for WasmCrawlConfig {
    fn from(val: kreuzcrawl::CrawlConfig) -> Self {
        Self {
            ...
            proxy: val.proxy.map(Into::into),
            bypass: val.bypass.as_ref().map(|v| format!("{v:?}")),
