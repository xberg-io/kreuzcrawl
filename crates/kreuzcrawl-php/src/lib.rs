...
// alef:hash:70d5bdac4eb919bb7c38c6a5b4e69b832914e2eca4d0305a505807b582e317ec
    ...
    fn from(val: AuthConfig) -> Self {
        match val.type_tag.as_str() {
            "basic" => kreuzcrawl::AuthConfig::Basic {
                username: val.username.map(Into::into).unwrap_or_default(),
                password: val.password.map(Into::into).unwrap_or_default(),
            },
            "bearer" => kreuzcrawl::AuthConfig::Bearer {
                token: val.token.map(Into::into).unwrap_or_default(),
            },
