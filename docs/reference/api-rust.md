...
## Rust API Reference <span class="version-badge">v0.3.0-rc.20</span>
...
| `eval_script`            | `Option<String>`              | `None`                          | JavaScript snippet evaluated after navigation completes. Scraping captures the native backend result in `ScrapeResult.browser.eval_result`; `interact()` runs it before actions on both browser backends without returning the script result. |
