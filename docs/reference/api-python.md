...
| `eval_script`            | `str \| None`         | `None`                         | JavaScript snippet evaluated after navigation completes. Scraping captures the native backend result in `ScrapeResult.browser.eval_result`; `interact()` runs it before actions on both browser backends without returning the script result. |
